...
{
    public static function create(?OfficeAccessTokenProvider $tokens = null): DevsysApiClient { $tokens ??= new OfficeAccessTokenProvider(new \App\Services\Oidc\OidcSessionService()); return new DevsysApiClient(new DevsysApiConfig(baseUrl:self::requiredEnvironment('DEVSYS_API_BASE_URL'),accessToken:$tokens->accessToken(),timeout:self::positiveFloat('DEVSYS_API_TIMEOUT',10.0),connectTimeout:self::positiveFloat('DEVSYS_API_CONNECT_TIMEOUT',5.0))); }
