...
final class OidcConfigTest extends TestCase
{
    ...
    public function testIssuerAndLogoutMustUseTheSameSupportedOrigin(string $issuer,string $logout,bool $valid):void{$_ENV['OFFICE_OIDC_ISSUER']=$issuer;$_ENV['OFFICE_CENTRAL_LOGOUT_URL']=$logout;$_ENV['OFFICE_CENTRAL_RP_LOGOUT_URL']=preg_replace('#/auth/logout$#','/auth/logout/rp',$logout);if(!$valid)$this->expectException(\RuntimeException::class);$config=OidcConfig::fromEnvironment();if($valid)self::assertSame($issuer,$config->issuer);}
