...
final class OidcClientTest extends TestCase
{
    ...
    public function testSuccessfulCallbackUsesBasicAndDoesNotStoreRefreshToken():void
    { $_SESSION['oidc_pending']=['state'=>'ok','nonce'=>'n','code_verifier'=>'v','return_to'=>'/dashboard','created_at'=>time()]; $http=new FakeOidcHttp(['status'=>200,'body'=>['access_token'=>'access','id_token'=>'','token_type'=>'Bearer','expires_in'=>3600,'scope'=>'openid read:clients','refresh_token'=>'never']]); $http->tokenResponse['body']['id_token']=$this->jwt($http); self::assertSame('/dashboard',$this->client($http)->callback(['state'=>'ok','code'=>'code'])); self::assertSame(['grant_type'=>'authorization_code','code'=>'code','redirect_uri'=>'https://office.test/callback','code_verifier'=>'v'],$http->fields); self::assertSame(['office-web','secret'],$http->basicAuth); self::assertArrayNotHasKey('refresh_token',$_SESSION); self::assertSame('access',$_SESSION['office_oauth']['access_token']); }
