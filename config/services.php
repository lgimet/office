...
use App\Services\Oidc\{CurlOidcHttpClient, LocalReturnToValidator, OidcClient, OidcConfig, OidcDiscoveryProvider, OidcHttpClientInterface, OidcIdTokenValidator, OidcSessionService, OidcTokenRefresher};

return static function (\App\Core\Container $container): void {
    $container->set(
        DevsysApiClient::class,
        static fn ($c): DevsysApiClient => DevsysApiProvider::create($c->get(OfficeAccessTokenProvider::class))
    );
    ...
    $container->set(OidcTokenRefresher::class, static fn ($c): OidcTokenRefresher => new OidcTokenRefresher($c->get(OidcHttpClientInterface::class), $c->get(OidcDiscoveryProvider::class), new OidcSessionService(), $c->get(OidcConfig::class)->clientId, $c->get(OidcConfig::class)->clientSecret));
